...
int main(int argc, char *argv[])
{
    ...
    for (size_t i=0; i<n_iter; i++) {
	    ...
	    do {
		    MPI_Isend( &msgleft, 1, MPI_INT, left_p, itag_l, ring_comm, &reqs[0] );
		    MPI_Isend( &msgright, 1, MPI_INT, right_p, itag_r, ring_comm, &reqs[1] );
		    
		    MPI_Recv( &tmpleft, 1, MPI_INT, right_p, MPI_ANY_TAG, ring_comm, &stats[0]);
		    MPI_Recv( &tmpright, 1, MPI_INT, left_p, MPI_ANY_TAG, ring_comm, &stats[1]);
